      ...
      <div style={{textAlign: 'center'}}>
        {
          status === 'ok' ? (
            <div className="dark-card center text-center">
              ...
              <input value={ password } onChange={ (event) => updatePassword(event.target.value) } className="dark-input" type="password" placeholder="New password" style={{width: '100%', marginTop: 50, marginBottom: 5}} />
              <input value={ confirmPassword } onChange={ (event) => updateConfirmPassword(event.target.value) } className="dark-input" type="password" placeholder="Confirm password" style={{width: '100%', marginTop: 5, marginBottom: 20}} />
              <div className="row" style={{ justifyContent: 'space-around' }}>
                <div style={{ display: 'table' }} onClick={() => verifyPassword()}><Button content="Submit" /></div>
              </div>
            </div>
          ) : (
            <div>
