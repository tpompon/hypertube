...
class PostersSlider extends React.Component {
  ...
  componentDidMount() {
    ...
    const controlLeft = document.getElementsByClassName("control-left")[
      number - 1
    ];
    const controlRight = document.getElementsByClassName("control-right")[
      number - 1
    ];
    const postersSlider = document.getElementsByClassName("posters-slider")[
      number - 1
    ];
    ...
    controlLeft.addEventListener(
      ...
      () => {
        let currentPos = parseInt(
          postersSlider.style.transform.substring(
        ...
        postersSlider.style.webkitTransform = `translateX(${currentPos +
          300}px)`;
      },
      false
    );
