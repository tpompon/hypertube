...
api.route("/auth/login").post((req, res, next) => {
...
api.route("/check").get((req, res) => {
  if (req.isAuthenticated())
    res.json({ status: "Access granted", user: req.user });
  else res.json({ status: "You can't access this page, please login before" });
});
// Route with authentication - example

api.route("/torrents/:search").get(async (req, res) => {
...
// Upload Avatar - API
api.route("/avatar/:username").post((req, res) => {
  const imageFile = req.files.file;
  const timestamp = Date.now();
  imageFile.mv(
    `${__dirname}/public/avatars/${req.params.username}_${timestamp}.jpg`,
    err => {
      if (err) res.json({ success: false });
      else
        res.json({
          success: true,
          file: `public/avatars/${req.params.username}_${timestamp}.jpg`
        });
    }
  );
...
api
  ...
  .post((req, res) => {
    ...
    newMovie.save(err => {
      if (err) res.json({ success: false });
      else res.json({ success: true, movie: newMovie });
    });
  });
...
api
  ...
  .put((req, res) => {
    ...
    if (req.body.name_fr) updateQuery.name_fr = req.body.name_fr;
    if (req.body.name_en) updateQuery.name_en = req.body.name_en;
...
// Movies comments
api.route("/movie/:id/comments").post((req, res) => {
